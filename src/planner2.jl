mutable struct POMCPOWHPlanner{P,NBU,C,NA,SE,IN,IV,SolverType,INF} <: Policy
    solver::SolverType
    problem::P
...
function POMCPOWHPlanner(solver, problem::POMDP)
...
Random.seed!(p::POMCPOWHPlanner, seed) = Random.seed!(p.solver.rng, seed)

function action_info(pomcp::POMCPOWHPlanner{P,NBU}, b; tree_in_info=false) where {P,NBU}
    A = actiontype(P)
...
action(pomcp::POMCPOWHPlanner, b) = first(action_info(pomcp, b))

function POMDPPolicies.actionvalues(p::POMCPOWHPlanner, b)
    tree = make_tree(p, b)
    search(p, tree)
    values = Vector{Union{Float64,Missing}}(missing, length(actions(p.problem)))
    for anode in tree.tried[1]
        a = tree.a_labels[anode]
        values[actionindex(p.problem, a)] = tree.v[anode][length(tree.v[anode])]
    end
    return values
end

function make_tree(p::POMCPOWHPlanner{P, NBU}, b) where {P, NBU}
    S = statetype(P)
    A = actiontype(P)
    O = obstype(P)
    B = belief_type(NBU,P)
...
function search(pomcp::POMCPOWHPlanner, tree::POMCPOWHTree, info::Dict{Symbol,Any}=Dict{Symbol,Any}())
    all_terminal = true
    # gc_enable(false)
    i = 0
    start_us = CPUtime_us()
