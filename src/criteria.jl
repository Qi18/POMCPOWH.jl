...
function select_best(crit::MaxUCB, h_node::POWHTreeObsNode,info::SimuInfo, rng) #UCT
    tree = h_node.tree
    h = h_node.node
...
function select_best(crit::MaxQ, h_node::POWHTreeObsNode,info::SimuInfo, rng) #选择Q值最大的
    tree = h_node.tree
    h = h_node.node
...
function select_best(crit::MaxTries, h_node::POWHTreeObsNode,info::SimuInfo, rng) #选择访问次数最多的
    tree = h_node.tree
    h = h_node.node
    best_node = first(tree.tried[h])
